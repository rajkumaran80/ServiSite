...
function GoogleReviewsSection({ content, primaryColor, googlePlaceId: placeIdProp, socialLinks, sectionBg, isDark = false, headingColor, bodyColor }: { content: GoogleReviewsContent; primaryColor?: string; googlePlaceId?: string; socialLinks?: any; sectionBg?: string; isDark?: boolean; headingColor?: string; bodyColor?: string }) {
  ...
  // Fetch Google reviews only if Google platform is enabled
  useEffect(() => {
    const fetchReviews = async () => {
      // Don't fetch Google reviews if platform is disabled
      if (content.platforms?.google === false) {
        setGoogleReviews([]);
        return;
      }
    ...
    fetchReviews();
  }, [content.maxReviews, content.minRating, content.platforms?.google]);
    ...
    <section className="pt-16 pb-10 lg:pt-20 lg:pb-12 overflow-hidden" style={{ backgroundColor: bgColor }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {(() => {
          ...
          const buttons: { label: string; url: string; icon: React.ReactNode; bg: string }[] = [];
