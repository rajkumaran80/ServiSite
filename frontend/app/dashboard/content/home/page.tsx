...
// ── Toggle section config ──────────────────────────────────────────────────────

const TOGGLES = [
  {
    key: 'showReviewsOnHome',
    label: 'Google Reviews',
    desc: 'Pull and display your Google reviews (requires Google Place ID in Settings)',
    defaultOn: true,
    manageKey: null as string | null,
  },
];
...
export default function HomeManagePage() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const t = tenant.themeSettings || {};
        const resolved: Record<string, boolean> = {};
        for (const tog of TOGGLES) {
          resolved[tog.key] = tog.defaultOn ? t[tog.key] !== false : t[tog.key] === true;
        }
        setToggles(resolved);
      } catch {
  ...
  const toggle = async (key: string, enabled: boolean) => {
    ...
    setSavingToggle(key);
    try {
      await api.put(`/tenant/${tenantId}`, { themeSettings: { [key]: enabled } });
    } catch {
      setToggles((prev) => ({ ...prev, [key]: !enabled }));
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 divide-y divide-gray-50">
        {TOGGLES.map(({ key, label, desc, manageKey }) => {
