    ...
    <div className="bg-white">
      ...
      {tenant.contactInfo && (tenant.whatsappNumber || tenant.contactInfo.phone || tenant.contactInfo.address || tenant.contactInfo.email) && (
        <section className="py-16 bg-gray-900 text-white relative overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-5" style={{ backgroundImage: `radial-gradient(circle at 20% 50%, ${primaryColor} 0%, transparent 50%), radial-gradient(circle at 80% 20%, ${primaryColor} 0%, transparent 40%)` }} />
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid md:grid-cols-2 gap-12 items-start">

              {/* Left — heading + contact + social */}
              <div>
                <p className="tenant-eyebrow mb-4" style={{ color: primaryColor }}>Get In Touch</p>
                <h2 className="tenant-h2-light text-3xl md:text-4xl text-white mb-8 leading-tight">
                  We&apos;d Love<br />to Hear From You
                </h2>
                <div className="space-y-4">
                  {(tenant.whatsappNumber || tenant.contactInfo.phone) && (
                    <a href={`tel:${tenant.whatsappNumber || tenant.contactInfo.phone}`}
                      className="flex items-center gap-4 group">
                      <span className="w-11 h-11 rounded-xl flex items-center justify-center text-lg flex-shrink-0 transition-transform group-hover:scale-110"
                        style={{ backgroundColor: `${primaryColor}25`, border: `1px solid ${primaryColor}40` }}>📞</span>
                      <div>
                  ...
                  {tenant.contactInfo.address && (
                    <div className="flex items-start gap-4">
                      <span className="w-11 h-11 rounded-xl flex items-center justify-center text-lg flex-shrink-0 mt-0.5"
                        style={{ backgroundColor: `${primaryColor}25`, border: `1px solid ${primaryColor}40` }}>📍</span>
                      <div>
                  ...
                  {tenant.contactInfo.email && (
                    <a href={`mailto:${tenant.contactInfo.email}`} className="flex items-center gap-4 group">
                      <span className="w-11 h-11 rounded-xl flex items-center justify-center text-lg flex-shrink-0 transition-transform group-hover:scale-110"
                        style={{ backgroundColor: `${primaryColor}25`, border: `1px solid ${primaryColor}40` }}>✉️</span>
                      <div>
                ...
                {tenant.whatsappNumber && (
                  <a href={`https://wa.me/${tenant.whatsappNumber.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hello! I'm interested in ${tenant.name}.`)}`}
                    ...
                    className="inline-flex items-center gap-2.5 mt-8 bg-[#25D366] hover:bg-[#1ebe5a] text-white font-bold px-6 py-3 rounded-xl transition-all hover:scale-[1.02] shadow-lg text-sm">
                ...
                {socialLinks && Object.values(socialLinks as Record<string, string | undefined>).some(Boolean) && (
                    ...
                    {(['instagram','facebook','tiktok','twitter','youtube'] as const).map((key) => {
                      ...
                      return (
                        <a key={key} href={url} target="_blank" rel="noopener noreferrer"
                          className="w-9 h-9 rounded-full flex items-center justify-center text-white/60 hover:text-white transition-all hover:scale-110"
                          style={{ backgroundColor: `${primaryColor}25`, border: `1px solid ${primaryColor}40` }}>
                          {icons[key]}
                        </a>
                      );
                    })}
              ...
              {(() => {
                ...
                return (
                  <div className="rounded-2xl p-6" style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <p className="tenant-eyebrow mb-5" style={{ color: primaryColor }}>Opening Hours</p>
                      ...
                      {groups.map(({days:gd,k}) => {
                        ...
                        return (
                          <div key={label} className="flex justify-between items-center py-3 border-b border-white/[0.06] last:border-0">
                            ...
                            <span className={`text-sm font-semibold ${k==='closed'?'text-red-400':'text-white'}`}>{hrs}</span>
                          </div>
                        );
                      })}
                    </div>
