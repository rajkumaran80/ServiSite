...
export default async function DjNightsPage({ params }: { params: { tenant: string } }) {
  const tenant = await getTenant(params.tenant);
  if (!tenant) notFound();

  const navPages = resolveNavPages(tenant.themeSettings?.navPages);
  if (!navPages['dj-nights']) notFound();

  const pageDef = getPredefinedPage('dj-nights')!;
  const entries = await getEntries(params.tenant, 'dj-nights');
